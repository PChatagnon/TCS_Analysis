...
int CS_Extraction_combine_Working_t_cs()
{
	...
	std::vector<double> nb_JPsi_Simu{};
	...
	Latex_Table_writter Latex_Table("", "/mnt/c/Users/pierrec/Desktop/TCS_Analysis/TCS_Analysis_2022/TCS_Analysis/CS_Extraction/t_cross_section", "t");
	...
	if (bint_2)
	{
		Eg_min = 9.28;	// 8.2;  // //8.205; //
		Eg_max = 10.36; // 9.28; //  // 10.6;  //
	}
	...
	if (inbending)
	{
		...
		samples.push_back({folder_pass2 + "Simulation/JPsi_Fall2018_45_022024.root", cs_no_rad_2018, color_JPsi, "Jpsi_Rad_Corr", charge_45_fall_18_inbending});
	...
	TCut kinematic_cut = "pass_EC_cut && Proton.Theta()*180./3.141592<35. && M>2.7 && (Electron.P() > 1.7) && (Positron.P() > 1.7) && positron_SF>0.15 && electron_SF>0.15 && ( Positron.P()<4.0 || (Positron.P()>4.0 && positron_score>0.05)) && ( Electron.P()<4.0 || (Electron.P()>4.0 && electron_score>0.05))  && positron_HTCC_ECAL_match==1. && electron_HTCC_ECAL_match==1.";
	...
	name_pdf = "CS_Extraction_combine_t_05_rad_new_";
	...
	for (int j = 0; j < samples.size(); j++)
	{
		...
		// MC_tree.push_back(MC_sample_tree);
		TTree *MC_sample_tree = (TTree *)sample_file->Get("tree_Gen");
		...
		// TTree *filtered_MC_tree = MC_sample_tree->CopyTree("((virtual_flux_Gen)>0)");
	...
	filtered_Data_tree->Draw("Epho>>Average_Eg", cut_Eg);
	...
	for (int i = 0; i < labels.size(); i++)
	{
		...
		filtered_Data_tree->Draw(label + ">>Data_hist", weight_data * data_cut * cut * exclusivity_cut * kinematic_cut);
		filtered_Data_tree->Draw(variable + ">>Average_variable", weight_data * data_cut * cut * exclusivity_cut * kinematic_cut);
		...
		for (int j = 0; j < samples.size(); j++)
		{
			...
			TCut weight = Form("%s*%f*%f*%f/(%i)", "weight", xsec, lumi_factor, lumi_sample, nbEvents_sample);

			// if (samples[j][3] != "BH TCSGen")weight = Form("%s*%s*%f*%f*%f/(%i*%s)", "weight", "virtual_flux_Frixione_Gen", xsec, lumi_factor, lumi_sample, nbEvents_sample, "virtual_flux_Gen");

			// TString hist_Acc = Form("sample_Acc_%s", samples[j][3].Data());
			...
			sample_hist->SetFillColorAlpha(std::stoi(samples[j][2].Data()), 0.65);
			...
			// legend->AddEntry(sample_hist, Form("%s (%3.1f)", samples[j][3].Data(), sample_hist->Integral()), "f1");
		}
	...
	for (int i = 0; i < labels_MC.size(); i++)
	{
		...
		for (int j = 0; j < samples.size(); j++)
		{
			...
			if (samples[j][3] == "J#psi")
			{
				...
				TString hist_name1 = Form("sample_hist1_%s", samples[j][3].Data());
				...
				TCut weight_acc = Form("%s*%f*%f*%f/(%i)", "weight", xsec, lumi_factor, lumi_sample, nbEvents_sample);
			...
			if (samples[j][3] == "Jpsi_Rad_Corr")
			{
				...
				TString hist_no_rad = Form("sample_no_rad_%s", samples[j][3].Data());
				TH1D *sample_no_rad = new TH1D(hist_no_rad, "", nbBins, min_histo, max_histo);

				TCut weight_no_rad = Form("%s*%f*%f*%f/(%i)", "weight", xsec, lumi_factor, lumi_sample, nbEvents_sample);
		...
		double CS_usual = 0.001 * nb_JPsi_Data[i] / ((variable_max - variable_min) * avg_flux * lumi_factor * Branching_ratio * (Acc)*w_c[i] * bin_volume_corr);
		cout << "CS using normal formula " << CS_usual << "\n";						  // remove lumi factor ?
		double error_CS_usual = CS_usual * (error_nb_JPsi_Data[i] / nb_JPsi_Data[i]); // 0.001 * error_nb_JPsi_Data[i] / ((variable_max - variable_min) * avg_flux * lumi_factor * Branching_ratio * (Acc)*w_c[i] * bin_volume_correction); //
		...
		double error_CS_comptage = 0.001 * sqrt(nb_JPsi_Data_C[i]) / (avg_flux * lumi_factor * Branching_ratio * (Acc)*w_c[i]); //
